...
function table_conversation(array) {
    ...
    array.forEach(item => {
      ...
      td4.innerHTML = `<div onclick="href_conv(${item.idConv})">VOIR</div>`;
...
async function verification_conversation(){
    var id = localStorage.getItem("id");
    let table = document.querySelector('.table-conversation');
    fetch("http://hms12432.hostmyservers.me/api/v1/conversations/", {
		method: 'GET',
		headers: {
			id:id,
		}
...
function href_conv(id_conv){
    window.open('./chat/index.html?id='+id_conv)
